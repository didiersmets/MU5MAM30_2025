...
static void update_all(PoissonSolverDirichlet &solver, Mesh &mesh, GPUMesh &mesh_gpu);
static void draw_scene(const Viewer &viewer, int shader,
		       const GPUMesh &gpu_mesh);
static void draw_gui(PoissonSolverDirichlet &solver);
...
bool new_rhs(PoissonSolverDirichlet &solver)
{
	srand((int)time(NULL));
...
int main(int argc, char **argv)
{
	...
	/* Prepare FEM data */
	PoissonSolverDirichlet solver(mesh, [](double x, double y, double z) {
		(void)z;
		return x * x - y * y;
	});
	if (!new_rhs(solver)) {
...
static void syntax(char *prg_name)
{
    printf("Syntax : %s (half_sphere | cube | cube2 | sphere | obj_filename) [n]\n", prg_name);
}

static int load_mesh(Mesh &mesh, int argc, char **argv)
{
    int res = -1;
    if (argc > 2 && strncmp(argv[1], "half_sphere", 11) == 0) {
        res = load_half_sphere(mesh, atoi(argv[2]));
    } else if (argc > 2 && strncmp(argv[1], "cube2", 5) == 0) {
...
static void update_all(PoissonSolverDirichlet &solver, Mesh &mesh, GPUMesh &gpu_mesh)
{

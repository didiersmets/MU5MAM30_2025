...
static int load_mesh(MyMesh &mesh, int argc, char **argv);
static void mymesh_to_mesh(const MyMesh &src, Mesh &dst);
...
static void update_all(MyMinimalGraphSolver &solver, Mesh &mesh, GPUMesh &mesh_gpu);
static void draw_scene(const Viewer &viewer, int shader, const GPUMesh &gpu_mesh);
static void draw_gui(MyMinimalGraphSolver &solver);
...
int main(int argc, char **argv)
{
	...
	MyMesh input_mesh;
	if (load_mesh(input_mesh, argc, argv)) {
		syntax(argv[0]);
		exit(EXIT_FAILURE);
	}
	...
	viewer.init("Minimal graph solver");
...
static void mymesh_to_mesh(const MyMesh &src, Mesh &dst)
{
	dst.positions.resize(src.vtx_count);
	dst.indices.resize(src.tri_count * 3);
	dst.attr.resize(src.vtx_count);

	for (size_t i = 0; i < src.vtx_count; ++i) {
		dst.positions[i] = {static_cast<float>(src.vertices[i].x),
				    static_cast<float>(src.vertices[i].y), 0.f};
		dst.attr[i] = 0.f;
	}
	for (size_t i = 0; i < src.tri_count; ++i) {
		dst.indices[3 * i + 0] = static_cast<uint32_t>(src.triangles[i].x);
		dst.indices[3 * i + 1] = static_cast<uint32_t>(src.triangles[i].y);
		dst.indices[3 * i + 2] = static_cast<uint32_t>(src.triangles[i].z);
	}
}
...
static void update_all(MyMinimalGraphSolver &solver, Mesh &mesh, GPUMesh &gpu_mesh)
{

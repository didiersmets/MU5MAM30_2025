...
// On construit cette fois-ci le graphe d'adjacence dans une fonction extérieure à partir de Edgeadjacency
//il est de taille le nombre de sommets + le nombre d'arêtes (les arêtes ayant déjà été indicées de manière unique
//grace à edgeadjacency) et pour un indice il renvoie toutes ses intéractions (arêtes et sommets)
std::vector<std::vector<uint32_t>> build_P2_adjacency_list(const Mesh &m, const EdgeAdjacency &edges)
{
    size_t vtx_count = m.vertex_count();
    size_t tri_count = m.triangle_count();
    size_t ddl_count = vtx_count + edges.num_edges;

    //table d'adjacence (triangulaire inférieure)
    ...
    for (size_t t = 0; t < tri_count; ++t) {
        ...
        // Si on n'a pas encore ajouté la connexion, on l'ajoute
        for (int i = 0; i < 6; ++i) {
            for (int j = 0; j < 6; ++j) {
                uint32_t row = ddls[i];
                uint32_t col = ddls[j];

                // On ne garde que la partie inférieure, pas de doublons grace à find
                if (col < row && !find(col, lower_adj[row].data(), lower_adj[row].size())) {
                    lower_adj[row].push_back(col);
                }//la fonction push_back permet de ne pas allouer de taille prédéfini à chaque adjacence
            }
        }
    }
...
void build_P2_CSRPattern(const Mesh &m, CSRPattern &P, const EdgeAdjacency &edges)
{
    ...
    //comme P1 mais cette fois ci on prévoit 15 connexions maximum comme beaucoup plus de DDL
    size_t max_nnz = 15 * tri_count + ddl_count;
    P.col.resize(max_nnz);
    //on utilise notre fonction pour avoir la table d'adjacence
    std::vector<std::vector<uint32_t>> lower_adj = build_P2_adjacency_list(m, edges);

    // On a toutes les connexions indicées par sommet donc on les met maintenant au format CSR (comme P1)
    size_t nnz = 0;
    for (size_t a = 0; a < ddl_count; ++a) {
        ...
        P.col[nnz++] = a; // On ajoute la diagonale
    }

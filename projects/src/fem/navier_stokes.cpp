...
extern void csr_cholesky_solve(const CSRMatrix& A, double* b, double* x, size_t N);
...
size_t NavierStokesSolver::compute_stream_function()
{
	...
	// Calcul du second membre : MOm = M * omega
	M.mvp(Om, MOm);

	// Résolution S * Psi = MOm par Cholesky CSR
	// S doit être de type CSRMatrix
	const CSRMatrix* S_csr = dynamic_cast<const CSRMatrix*>(&S);
	if (!S_csr) {
		// Gestion d'erreur si S n'est pas CSR
		assert(false && "S n'est pas une CSRMatrix");
		return 0;
	}
	csr_cholesky_solve(*S_csr, MOm, Psi, N);
	return 1; // 1 itération, car résolution directe
}

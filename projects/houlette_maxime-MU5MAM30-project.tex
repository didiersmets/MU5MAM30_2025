...
\begin{itemize}
    ...
    \item[$\bullet$] $\mathcal{P}_K$ is the space $\mathbb{P}_k(K)$ of polynomials of total degree at most $k$, which has dimension $\binom{d+k}{k}$ (or equivalently $\binom{d+k}{d}$),
    \item[$\bullet$] $\Sigma_K$ is the set of linear forms associated with the evaluation of an element of $\mathcal{P}_K$ at the nodes of the principal lattice of order $k$.
\end{itemize}
...
We compute the $\mathbb{P}_2$ shape functions associated with the triangle $K$ shown in Figure 1. We provide the computational details for the shape function associated with $\bm x_1$ in barycentric coordinates, denoted as $\tilde{\varphi}_{\bm x_1}$. That is to say that, considering the reference triangle $\hat{K} := ((0, 0), \, (1, 0), \, (0, 1))$, and knowing that every $\bm x\in K$ can be uniquely represented with $(\lambda_1(\bm x), \lambda_2(\bm x)) \in \hat{K}$ (as $\lambda_3(\bm x) = 1 - \lambda_1(\bm x) - \lambda_2(\bm x)$), we can write 

$$\forall \bm x \in K, \, \varphi_{\bm x_1}(\bm x) = \tilde{\varphi}_{\bm x_1}(\lambda_1(\bm x), \lambda_2(\bm x))$$
...
where, for $\bm x \in K$ and the corresponding $(\lambda_1(\bm x), \lambda_2(\bm x)) \in \hat{K}$, we have $f(\bm x) = \tilde{f}(\lambda_1(\bm x), \lambda_2(\bm x))$ (as introduced before), and $\Lambda = \|\bm n \|$, with $\bm n = (\bm x_2 - \bm x_1) \times (\bm x_3 - \bm x_1)$, is related to the actual geometric shape of $K$. This last statement follows from the fact that any $\bm x \in K$ can be expressed as:
